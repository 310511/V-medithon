#!/usr/bin/env python3
"""
Test script to check edge cases that might cause the same diagnosis
"""

def test_edge_cases():
    print("🧪 Testing Edge Cases for Diagnosis Variety")
    print("=" * 60)
    ...
    # Test edge cases that might cause the same diagnosis
    edge_cases = [
        "I don't feel well",
    ...
    print("Testing edge cases (inputs without specific symptoms):")
    print("-" * 60)
    ...
    for i, test_input in enumerate(edge_cases, 1):
        ...
        print(f"   Symptoms found: {result.get('symptoms', 'None')}")
    ...
    if len(unique_diagnoses) == 1:
        print("\n❌ ISSUE FOUND: All edge cases give the same diagnosis!")
        print("   This explains why the user sees the same output for all inputs.")
        print("   The system needs to handle non-specific inputs better.")
    else:
        print("\n✅ Edge cases are handled with variety")
